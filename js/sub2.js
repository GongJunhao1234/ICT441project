...
function Validate()
{
    ...
    //Detection firstName Check if it is all letters and if it does not cause an error.
    if (firstNameValue === '') {
    ...
    // Detection phone Check if all numbers are present, otherwise an error will be reported.
    if (phoneValue === '') {
    ...
    //Detection email Check if the email input is correct, and no error will be reported.
    ...
    // Detection gender The detection is based on gender selection, not error reporting.
    if (genderValue === '') {
    ...
    // Detection enquiry  Check if the comments are filled in and if they exceed 130 words, and do not report an error.
    if (enquiryValue === '') {
